...
function UPCP_Adjust_Sidebar_Counts(min_price, max_price, CatValues, SubCatValues, TagBoxValues, CFBoxValues) {
	...
	jQuery('.prod-cat-sidebar-category label').each(function(index, el) {
		...
		if (Sidebar_Counts['Categories'][ID] === undefined) {jQuery(this).find('span span').html('(0)')}
		else {jQuery(this).find('span span').html('(' + Sidebar_Counts['Categories'][ID] + ')');}
	});

	jQuery('.prod-cat-sidebar-subcategory label').each(function(index, el) {
		var ID = jQuery(this).attr('for').substring(4);
		if (Sidebar_Counts['SubCategories'][ID] === undefined) {jQuery(this).find('span span').html('(0)')}
		else {jQuery(this).find('span span').html('(' + Sidebar_Counts['SubCategories'][ID] + ')');}
	});
	...
	jQuery('.prod-cat-sidebar-cf-content').each(function(index, el) {
		...
		if (jQuery(this).find('select').length == 0) {
			jQuery(this).find('label').each(function() {
				...
				if (Sidebar_Counts['CustomFields'][CF_ID] !== undefined && CF_Value in Sidebar_Counts['CustomFields'][CF_ID]) {jQuery(this).find('span span').html('(' + Sidebar_Counts['CustomFields'][CF_ID][CF_Value] + ')');}
				else {jQuery(this).find('span span').html('(0)');}
			});
		}
		else {
			jQuery(this).find('option').each(function() {
				var CF_Value = jQuery(this).val(); console.log(CF_Value);
				if (CF_Value != 'All' && Sidebar_Counts['CustomFields'][CF_ID] !== undefined && CF_Value in Sidebar_Counts['CustomFields'][CF_ID]) {
					...
					jQuery(this).html(Option_Text.substring(0, Option_Text.indexOf('(')) + '(' + Sidebar_Counts['CustomFields'][CF_ID][CF_Value] + ')');
				}
